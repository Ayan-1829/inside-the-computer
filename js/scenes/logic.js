...
/* ---------------- Physical IC pinout ---------------- */
SCENES.ic = (n) => {
  const c = CHIPS[n.id], pins = c.labels.length, per = pins/2, x0 = 250, w = 500, pitch = w/per;
  const pl = l => l.startsWith('~') ? `<tspan class="ol">${esc(l.slice(1))}</tspan>` : esc(l);
  const kind = k => ({i:'in',o:'out',p:'pwr'})[k];
  ...
  s += T(500,320,c.part,'t t-xl t-inv t-mid') + T(500,352,c.desc,'t t-sm t-inv t-mid','style="opacity:.75"');
  ...
  units.forEach(([name],i) => { s += `<g class="ctl btn-s ukey" data-u="${i}" role="button" tabindex="0" aria-label="Highlight ${name} pins">${R(bx+i*(bw+10),510,bw,44,10,'')}${T(bx+i*(bw+10)+bw/2,537,name,'')}</g>`; });
  s += `<g class="bg">${T(500,600,'','t t-sm t-mid','id="unit-info"')}
    <circle cx="350" cy="648" r="7" style="fill:var(--accent)"/>${T(364,653,'Input','t t-xs')}
    <circle cx="440" cy="648" r="7" class="m-on"/>${T(454,653,'Output','t t-xs')}
    <circle cx="540" cy="648" r="7" style="fill:#B24A3A"/>${T(554,653,'Power (+5 V / ground)','t t-xs')}</g>`;
  const lst = a => a.length > 1 ? a.slice(0,-1).join(', ') + ' and ' + a[a.length-1] : a[0];
  const describe = (name, ps) => {
    if (name === 'Power') return `Pin ${pins} is +5 V (VCC) and pin ${pins/2} is ground (GND).`;
    const ins = ps.filter(p => c.kinds[p-1] === 'i'), outs = ps.filter(p => c.kinds[p-1] === 'o'), parts = [];
    if (ins.length) parts.push(`input${ins.length>1?'s':''} on pin${ins.length>1?'s':''} ${lst(ins)}`);
    if (outs.length) parts.push(`output${outs.length>1?'s':''} on pin${outs.length>1?'s':''} ${lst(outs)}`);
    return `${/^\d/.test(name) ? 'Inverter ' + name : name}: ${parts.join(', ')}.`;
  };
  return {svg:s, init: el => {
    const info = el.querySelector('#unit-info'); let locked = null;
    const show = i => {
      ...
      if (i === null){ info.textContent = `Choose a ${c.gate ? 'gate' : 'section'} to see which pins it uses. The chip needs +5 V on pin ${pins} and ground on pin ${pins/2}.`; return; }
      const [name, ps] = units[i];
      ps.forEach(p => el.querySelector(`.pin[data-pin="${p}"]`).classList.add('lit'));
      el.querySelector(`.ukey[data-u="${i}"]`).classList.add('lit','on');
      info.textContent = describe(name, ps);
    };
