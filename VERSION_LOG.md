...
> Recheck if any important step is missed or not. If it is okay then push it to the github. before that add a feedback button to the bottom right. it should open a message box. The message can be stored in the sheet that I have provided.

**Changes**
- **Feedback button and message box** (`js/feedback.js`, a shared file like `analytics.js`, copied unchanged into other projects): a small "Feedback" button at the bottom right (in the footer row on desktop, a floating pill on phones) opens a message box with a text area (1000 characters), a note not to include personal details, and Send / Cancel.
- **Stored in the existing Google Sheet, with no secret in the public code**: the message goes to the same Cloudflare Worker as the analytics, which adds the real secret and passes it to the Apps Script; a new "Feedback" tab (Timestamp, Project, Page, Device, Message) collects them, separate from the visit counts. Both backend files were updated (kept outside the repo, since the Worker file contains the private Apps Script URL): the Worker accepts a `feedback` event (trimmed, capped at 1000 characters, project checked, a hidden bot-trap field, request size limited) and answers a plain GET so the page can tell it supports feedback; the Apps Script writes the row and stores any message that starts with = + - or @ as plain text so it cannot run as a spreadsheet formula.
- **Nothing is lost or falsely confirmed**: the page asks the Worker once (when the box is first opened) whether it can store feedback; if not, the box says feedback isn't available and Send stays disabled. "Thank you" only appears when the Worker confirms. A failed send keeps the visitor's text, and there is a 20-second wait between messages.
- **Easier to notice**: the Feedback button is larger and filled with the accent colour, with a ring, a shadow (dark in light mode, a light glow in dark mode) and a short pulse when the page loads (skipped for reduced-motion); the footer is a little taller to hold it. The switch-on steps pop-up now has an accent border and header and the same light/dark shadow treatment, so it stands out from the diagram.
- **Walkthrough recheck** against how a PC really starts: the sequence holds up; two additions were made: the CPU is powered through the motherboard's voltage regulators (VRM), and the first picture on the monitor (the maker's logo, once the graphics card is ready) now appears in the hardware-initialisation step.

### 5.21.0: Power button switch-on walkthrough

**Prompt** (2026-09-20, exact time not in the saved record):
> Like the mic/camera (input) device the flow of data is shown, I want the same for switching the computer one. There is a power button on the top right of the tower. Make a flow how signal is passed. For the flow add short note on top about what is happening. [...] Add bios to the motherboard... add necessary components if need to showcase the flow. Show the steps one by one on the right description box when it is not in full-screen mode. Otherwise make a pop-up tab that shows the steps, new steps should come one by one after the old step but the steps remain. Add right button details part for the power button too like other. [...] rearrange the components of the motherboard to be spreaded evenly. Add pause, step button on the pop-up tab [...] for step one and two power is supplied to the motherboard always right? then avoid dot there [...] Flow should start from the power button. [...] when multiple components get the flow then show it with multiple dots [...] make the buttons small and take it beside the power-on steps text. Keep more space in the description. Try to reduce the steps if possible [...] Bring the pop-up tab near the tower a bit.

**Changes**
- **Power button is a real part**: new `power-button` node (an input device, with a page, hover card and a right-click card with a "Power on" button). Pressing it in the diagram (click, Enter or Space) plays the walkthrough; pressing it again stops it.
- **9 steps** (down from the 18 in the original list; related steps were merged and the text kept): standby power (AC, PSU, +5VSB, motherboard); the button press (PWR_SW#) reaching the power logic; PS_ON# to the PSU; the main power rails; PWR_OK and CPU reset release; CPU to UEFI; hardware initialisation and POST; boot device, bootloader and OS kernel; the OS running, drivers and the desktop.
- **Several dots at once where a signal reaches several parts**: the PSU's power fans out to the board, CPU, RAM, chipset, BIOS, fans, graphics card and drives (the last two along their own cables); the firmware's initialisation reaches RAM, GPU, drives and USB; and at the end the OS's signals fan out from the I/O port to the monitor, keyboard, mouse, webcam, mic, joystick, game controller, printer and speakers. Dots share the start of their path, so they look like one dot that splits. Dots move at a constant speed, so short hops along a cable aren't slow.
- **Standby power is a state, not a dot**: step 1 has no moving dot. The wall cord, the PSU-to-24-pin cable and the PWR_SW# wire are lit, because that power is always there. Pressing the button (step 2) drops the wire dark (LOW) and the button visibly presses in; it returns HIGH afterwards, and the main rails (the cables to the motherboard, graphics card and drives) light up from step 4. The moving dot starts at the power button.
- **Where the steps show**: a one-line note at the top of the diagram ("4/9 +12 V, +5 V and +3.3 V go out to every part") always; the full text of every step, added one by one and kept, in the details panel on the button's page; and in a pop-up when the panel isn't visible (full screen, or "Hide details"). The pop-up is placed just left of the tower (measured from the drawing, so it follows the layout and full screen) and leaves the tower uncovered. Leaving full screen moves the same steps back into the panel.
- **Small controls**: Power on / Stop, Back, Pause / Play and Step are small buttons on the same row as the "Power-on steps" heading in the panel (leaving the space for the text), and in the pop-up's header. Pause holds after the current step's animation (the note shows "paused"); Step and Back move one step and stay paused (Back also removes the later steps from the log); Step from rest starts a paused walkthrough.
- **New parts on the tower's motherboard**: BIOS chip (UEFI), chipset (standing in for the board's power-control logic), the 24-pin ATX connector (where PS_ON# and PWR_OK travel), the PWR_SW# wire from the button, and the wall plug on the power supply's cord. The board is laid out evenly (RAM, CPU and a BIOS-over-chipset column with equal gaps, on one centre line) with traces between them; the firmware chip reaches the CPU through the chipset. BIOS, Chipset and 24-pin are the existing parts, so clicking them opens the motherboard view with that part focused. The legend moved right to clear the plug, and the drawing is 1.2% wider so the tower's right edge and the button aren't clipped.
- Leaving the computer's diagram, or opening another part, ends a running walkthrough cleanly. On a phone the page doesn't scroll to follow the steps (they collect in the panel below); the top note carries the story.
- Webcam and mic flows behave as before (they also now stop cleanly if you navigate away mid-flow).
